            ...
            <tbody id="sortable-list" class="divide-y divide-gray-700">
                <?php if (count($products) > 0): ?>
                    <?php foreach ($products as $product): ?>
                        <tr class="hover:bg-gray-750 transition-colors sortable-row cursor-grab active:cursor-grabbing" data-id="<?php echo $product['id']; ?>">
    ...
    Sortable.create(el, {
        ...
        onEnd: function() {
            ...
            fetch('/admin/api/update_sort.php', {
                ...
                body: JSON.stringify({ table: 'products', order: order })
            })
            .then(res => res.json())
            .then(data => {
                if (data.success) {
